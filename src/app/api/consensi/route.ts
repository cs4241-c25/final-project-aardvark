...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const consensusData of allConsensuses) {
      try {
        const allSubmissions = await gameData.getByConsensusId(
          consensusData._id.toString()
        );
        ...
        allSubmissions.forEach((submission) => {
          Object.entries(submission.submission).forEach(([option, rank]) => {
            if (validOptions.has(option)) {
              ...
              consensusValues[option] +=
                scoreMap[rank as keyof typeof scoreMap] || 0;
            }
          });
        });

        // Sort results: first by score (descending), then by option lexicographically
        const sortedConsensus = Object.entries(consensusValues)
          .sort(
        ...
        consensiList.push({
          ...
          overall: sortedConsensus,
          numSubmissions: allSubmissions.length,
        });
      } catch (error) {
        console.error(
          `Error processing consensus ${consensusData._id}:`,
          error
        );
    ...
    if (resolvedConsensiList.length === 0) {
      return NextResponse.json(
        { error: "No consensuses found with submissions." },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Error getting Consensi List" },
      { status: 500 }
    );
  }
}
