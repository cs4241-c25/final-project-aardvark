...
const ConsensusItem: React.FC<{
  ...
  return (
    <div
      ...
      className="p-4 border rounded-lg shadow-sm hover:shadow-md transition mb-4 cursor-pointer"
    >
      <button
              ...
              <div className={`flex-1 ${!hasUserSubmission ? "w-full" : ""}`}>
                ...
                {consensus.overall.map(({ option }) => (
                  <AnimatedOption
                    ...
                    option={option}
                    colorClass={`bg-game${
                      consensus.options[option].charAt(0).toUpperCase() +
                      consensus.options[option].slice(1)
                    }`}
                  />
                ))}
              </div>
