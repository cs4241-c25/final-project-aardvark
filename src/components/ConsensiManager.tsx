...
const ConsensiManager: React.FC = () => {
  const form = useForm({
    defaultValues: {
      author: "",
      category: "",
  ...
  const [aiLists, setAILists] = useState<ConsensiSuggestion[]>(data);
  ...
  const onSubmit = async (values: {
    ...
    try {
      ...
      await axios
        ...
        .then((response) => {
          setConsensusQueue((prevQueue) => [
            ...prevQueue,
            response.data.consensusData,
          ]);
        })
        .catch((error) => {
  ...
  return (
    <Tabs
      defaultValue="user"
      className="w-[600px] self-center justify-center pt-5"
    >
                ...
                {userSuggestions.map((record) => (
                  <TableRow key={record._id?.toString()}>
                    ...
                    <TableCell className="flex gap-2">
                      <Button
                        variant="primary"
                        className="text-green-500 border-green-500 bg-transparent p-1"
                        onClick={async () => {
                          try {
                      ...
                      <Button
                        variant="primary"
                        className="text-red-500 border-red-500 bg-transparent p-1"
                        onClick={async () => {
                          try {
                            await handleDeleteConsensi(record._id!.toString());
                            setUserSuggestions((prevData) =>
          ...
          <CardContent className="space-y-2">
            ...
            <Table>
              ...
              <TableBody>
                {aiLists.map((record) => (
                  <TableRow
                    key={record.author + record.category + record.options}
                  >
                    <TableCell>{record.category}</TableCell>
                    <TableCell>{record.author}</TableCell>
                    <TableCell>{record.options.join(", ")}</TableCell>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          try {
                            await handleApproveConsensi(record);
                            setAILists((aiLists) =>
                              aiLists.filter((item) => item !== record)
                            );
                          } catch (error) {
      ...
      <TabsContent value="consensus">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Create Custom Consensus</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <Form {...form}>
              <form
              ...
              >
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          {...field}
                          placeholder="Enter author name (or leave blank for Anonymous)"
                        />
                      </FormControl>
                    </FormItem>
                  )}
                ...
                <Button
                  type="submit"
                  className="w-full outline outline-1 outline-black"
                >
                  Submit Suggestion
                </Button>
              </form>
              ...
              <TableBody>
                {consensusQueue.map((record) => (
                  <TableRow key={record.consensusNum}>
                    ...
                    <TableCell>{record.category}</TableCell>
                    <TableCell>
                      {Object.keys(record.options).join(", ")}
                    </TableCell>
